...
function genIndex(articles) {
  var articlesJson = JSON.stringify(articles.map(function(a) {
    return { id: a.id, title: a.title, date: a.dateFormatted, excerpt: a.excerpt, tags: a.tags, tagsHtml: a.tagsHtml, url: POST_DIR + '/post-' + a.id + '.html' };
  }));
  ...
  return [
    ...
    '    <meta name="description" content="影客笔记">',
    '    <title>laowei</title>',
    '    <style>' + style + '</style>',
    '</head>',
    '<body>',
